...
from evaluation.evaluation import _log_progress, _safe_retrieve, compute_retrieval_metrics
...
def _load_split(dataset_id: str, config_name: str, split_name: str):
    ...
    if ds_path.exists() and ds_path.is_dir():
        try:
            local_obj = load_from_disk(str(ds_path))
            if hasattr(local_obj, "column_names"):
                return local_obj
            for key in (split_name, config_name, "train"):
                if key in local_obj:
                    return local_obj[key]
            first_key = next(iter(local_obj.keys()), None)
            if first_key is not None:
                return local_obj[first_key]
        except Exception:
            pass
        raise RuntimeError(f"Could not load local dataset split '{split_name}' from '{dataset_id}'.")
...
def _build_mteb_retriever(
    ...
    if faiss_cache_path.exists():
        ...
        try:
            ...
            if int(getattr(faiss_index, "d", 0)) <= 0 or int(getattr(faiss_index, "ntotal", -1)) != len(chunks):
                raise RuntimeError("Cached FAISS index invalid")
        except Exception as exc:
    ...
    if faiss_index is None:
        ...
        faiss.write_index(faiss_index, str(faiss_cache_path))
...
def _build_mteb_splade_retriever(
    *,
    ...
    corpus_ds = _load_split(dataset_id, "en-corpus", "test")
    chunks, _ = _build_mteb_chunks(corpus_ds, max_corpus)
    _log_progress(f"Building SPLADE index for {len(chunks)} chunks...")
