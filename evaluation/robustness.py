"""Robustness analysis: bootstrap CI, paired permutation tests, error categorisation.

cmd_robustness is called by `main.py evaluate --with-robustness`.
"""
...
from evaluation.retrieval_eval import _build_retrievers_for_model
...
def _error_categories(
    ...
    """Classify each query as success / partial / late_hit / complete_miss."""
    ...
    for item in query_instances:
        ...
        for chunk in result.ranked_chunks:
            if chunk.document not in seen_set:
                seen.append(chunk.document)
                seen_set.add(chunk.document)
    ...
    counts_df = (
        per_query_df["category"].value_counts(dropna=False)
...
def cmd_robustness(args: argparse.Namespace) -> None:
    """Run publishability-focused robustness analysis for one model."""
    args.output_dir.mkdir(parents=True, exist_ok=True)

    idx_prefix = INDEX_DIR / args.model
    if not (
        Path(str(idx_prefix) + "_faiss.index").exists()
        and Path(str(idx_prefix) + "_bm25.pkl").exists()
    ):
        raise FileNotFoundError(
            f"Indices for model '{args.model}' are missing. "
            f"Build first with: python main.py build -i outputs/evidence.csv -m {args.model}"
        )
    ...
    retrievers = _build_retrievers_for_model(
        args.model, top_k=args.top_k, rerank_top=args.rerank_top, reranker=reranker,
    )
    ...
    for method_name, retriever in retrievers.items():
        scores = per_query_retrieval_scores(
            retriever, gold_path=args.gold_csv, k=args.k,
            top_k_retrieve=max(args.top_k * 3, 30), rerank_top=args.rerank_top, level="document",
        )
        n = len(scores["hit"])
        if n != len(gold_queries):
            raise RuntimeError(f"Per-query score length mismatch: scores={n}, gold_queries={len(gold_queries)}")

        for i in range(n):
            per_query_long_rows.append({
                "model_key": args.model, "method": method_name, "query_index": i,
                "query": gold_queries[i],
                "hit": float(scores["hit"][i]), "recall": float(scores["recall"][i]),
                "precision": float(scores["precision"][i]), "mrr": float(scores["mrr"][i]),
                "ap": float(scores["ap"][i]), "ndcg": float(scores["ndcg"][i]),
        ...
        for metric in ("hit", "mrr", "ndcg"):
            ...
            ci_rows.append({
                "model_key": args.model, "method": method_name, "k": args.k, "metric": metric,
                "mean": mean_val, "ci95_low": ci_lo, "ci95_high": ci_hi, "num_queries": len(scores[metric]),
            })
        ci_rows.append({
            "model_key": args.model, "method": method_name, "k": args.k, "metric": "mean_rank",
            "mean": _mean_or_inf(scores["rank"]), "ci95_low": np.nan, "ci95_high": np.nan,
    ...
    for a, b in combinations(methods, 2):
        ...
        for metric in ("mrr", "ndcg", "hit"):
            ...
            p_val = paired_permutation_test(scores_a, scores_b, n_permutations=10_000, rng_seed=42)
            delta = float(dfa[metric].mean() - dfb[metric].mean())
            effect_dz = _paired_effect_size_dz(scores_a, scores_b)
            pair_rows.append({
                "model_key": args.model, "k": args.k, "metric": metric,
                "method_a": a, "method_b": b, "delta_mean": delta,
                "p_value": p_val, "effect_size_dz": effect_dz,
    ...
    mean_by_method_metric = per_query_df.groupby("method")[["hit", "mrr", "ndcg", "rank"]].mean().to_dict("index")
    ablations = [
    ...
    for base, variant, name in ablations:
        ...
        abl_rows.append({
            "model_key": args.model, "ablation": name, "base": base, "variant": variant,
            "delta_hit": float(v["hit"] - b["hit"]), "delta_mrr": float(v["mrr"] - b["mrr"]),
            "delta_ndcg": float(v["ndcg"] - b["ndcg"]), "delta_mean_rank": float(v["rank"] - b["rank"]),
        })
    ...
    per_query_error_df, error_counts_df = _error_categories(
        best_retriever, args.gold_csv, k_main=args.k, top_k=args.top_k, rerank_top=args.rerank_top,
    )
    ...
    per_query_error_df[per_query_error_df["category"].isin(["late_hit", "complete_miss"])].to_csv(negative_csv, index=False)

    means_table = per_query_df.groupby("method")[["hit", "mrr", "ndcg", "rank"]].mean().sort_values("ndcg", ascending=False)
    interp_lines = [f"Robustness interpretation for model={args.model}, k={args.k}", "", "Method means (hit, mrr, ndcg, rank):"]
    for method_name, row in means_table.iterrows():
        interp_lines.append(f"- {method_name}: hit={row['hit']:.4f}, mrr={row['mrr']:.4f}, ndcg={row['ndcg']:.4f}, rank={row['rank']:.2f}")
    if not comparison_report_df.empty:
        interp_lines.extend(["", "Paired differences significant after Holm correction (alpha=0.05):"])
    ...
    summary = {
        ...
        "model": args.model, "k": args.k, "best_method_by_ndcg": best_method,
        "outputs": {
            "per_query_scores": str(per_query_csv), "ci_table": str(ci_csv),
            "pairwise_permutation": str(pvals_csv), "comparison_report": str(comparison_report_csv),
            "ablation_deltas": str(ablation_csv), "error_queries": str(errors_csv),
            "error_summary": str(errors_counts_csv), "negative_cases": str(negative_csv),
